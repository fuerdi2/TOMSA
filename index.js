...
//         getData(socket.id, 'culto', 'point');
//         getData(socket.id, 'cultura', 'point');
//         getData(socket.id, 'cysf', 'point');
//         getData(socket.id, 'deportes', 'point');
//         getData(socket.id, 'educacion', 'point');
//         getData(socket.id, 'edusup', 'point');
//         getData(socket.id, 'recintos_feriales', 'point');
//         getData(socket.id, 'sa', 'point');
//         getData(socket.id, 'salud', 'point');
//         getData(socket.id, 'seguridad', 'point');
//         getData(socket.id, 'land', 'point');
//         getState(socket.id, 0);
//         console.log(':: This is a ' + glbs.INITIALIZE + ' request...');
//     });
...
//     var query = 'SELECT *, ST_AsText(geom) AS wkt FROM ' + table + ' WHERE step = \'' + step + '\'';
//     makeQuery(query).then(function (result){
//         var msg = {};
//         msg.type = 'point';
//         msg.features = [];
...
//     var query = 'SELECT i.si,i.step,u.*, ST_AsText(u.geom) AS wkt FROM indicator2 i INNER JOIN upz u ON i."idUPZ" = u.gid WHERE step = ' + time;
//     console.log(query);
//     makeQuery(query).then(function (json){
//         var msg = {};
//         msg.type = 'multipolygon';
//         msg.features = [];
//         for (var i = 0; i < json.length; i++) {
